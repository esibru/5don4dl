...
Remarque comme l'ajout de couples clé/valeur est simple et élégant. Attention cependant: si la collection utilisée pour l'insertion spécifie un schéma, les données insérées doivent satisfaire toutes les contraintes du schéma. Ceci peut donner lieu à des conversions entre les types de données MongoDB et les types de données Java. Dans l'exemple ci-dessus, nous nous sommes volontairement limités à des String/des entiers.
