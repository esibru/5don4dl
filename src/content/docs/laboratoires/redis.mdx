...
jedis.set("g12345", Integer.toString(14));
...
2. Créez un package `model` ainsi qu'une classe `RateLimiter`. Cette classe comportera trois attributs: 
   * le nombre `maxReq` maximal de requête acceptée par intervalle de temps, 
   * la durée en secondes `nbSeq` de l'intervalle de temps, 
   * ainsi qu'un objet `JedisPool` qui sera utilisé pour la connection au serveur Redis.
...
1. Créez un fichier texte nommé `redis.conf` et contenant (pour le moment) uniquement la ligne:
   ...
   ``` conf showLineNumbers=false title=redis.conf
   SAVE 20 10
   ```
...
``` text showLineNumbers=false title=redis.conf
SAVE ""
``` 
...
``` text showLineNumbers=false config title=redis.conf
APPENDONLY YES
```
...
``` text showLineNumbers=false title=redis.config
APPENDONLY YES
