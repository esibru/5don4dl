...
Nous allons ici vous demander de développer une petite classe Java simulant un rate limiter au moyen de Redis: a chaque utilisateur (identifé par un `userId`) sera associé une clé sur le serveur Redis nommée `limit:userId` qui enregistrera le nombre de requêtes faites par l'utilisateur au cours d'un intervalle de temps donné.
...
3. Ajoutez un constructeur a deux paramètres à `RateLimiter` permettant d'initialiser `maxReq` et `nbSeq`. Initialisez le JedisPool avec le serveur Redis local et le numéro de port standard 6379 (ceci peut éventuellent se configurer dans le constructeur si nécessaire, mais nous nous simplifions la vie ici).
...
Testez votre classe `RateLimiter` au moyen d'une classe principale pertinente (n'oubliez évidemment pas de lancer le serveur Redis au pré
alable).
...
**Exemple d'utilisation **: Redis est utilisée comme tampon (mémoire cache) entre l'application qu'on souhaite développer et une base de données utilisée pour la persistance (celle-ci peut par exemple être une DB SQL standard).
...
En cas de crash du serveur Redis, seul le cache est perdu ce qui n'est pas genant (les données sont de toute façon stockées de façon sûr par le SGBD utilisé pour la persistance).
