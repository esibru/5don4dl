...
En cas de crash du serveur Redis, seul le cache est perdu ce qui n'est pas genant (les données sont de toute façon stockées de façon sûre par le SGBD utilisé pour la persistance).
