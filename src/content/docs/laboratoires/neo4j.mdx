...
4. Les liens et les sommets possèdent des **attributs** (par exemple, dans le cas d'une personne, le nom et la date de naissance).
...
La modélisation sous forme de graphe n'est pas adaptée à toutes les applications, une réflexion doit être menée au préalable pour savoir si l'application  s'y prête ou non.

<Card title="Modéliser les données sous forme de graphe" icon="pencil">
Les données cinématographiques (ci-dessus) fournissent un exemple d'application des bases de données orientées graphes. Fournissez deux exemples supplémentaires d'application où les graphes apparaissent naturellement (aidez-vous d'internet au besoin). Dans chaque cas, précisez clairement ce que les sommets et les arcs représentent.
</Card>
...
Comme vous avez pu le constater ci-dessus, les requêtes SQL nécessaires pour parcourir le graphe deviennent vite longues et fastidieuses. L'utilisation d'un SGBD dédié à la manipulation des graphes prend alors ici tout son intérêt.
...
<Aside icon="seti:docker" title="Installation">
La commande Docker à utiliser est :
...
```
* `neo4j/myPassword` définit le nom d'utilisateur et le mot de passe.
   * Remplacez `myPassword` par un mot de passe de votre choix.
   * L'utilisateur `neo4j` doit rester inchangé.
* Utilisez un autre nom de conteneur que MyNeo4j si vous le désirez.
...
Nous allons tout d'abord apprendre à ajouter des noeuds (sommets) et des relations (arcs) à notre graphe au moyen du langage Cypher. Pour ce faire, le plus simple est d'utiliser la zone de texte située au dessus de l'interface web:

![image](images/neo4jCypher.png)

Mais il est aussi possible de se connecter à Neo4j en ligne de commande:

``` text showLineNumbers=false title=neo4j$ 
docker exec -it MyNeo4j cypher-shell -u neo4j -p votre_mot_de_passe
```
...
``` text showLineNumbers=false title=neo4j$ 
MATCH (c1:City{id:1}), (c2:City{id:2}) CREATE (c1) - [:HIGHWAY{distance:70}]->(c2)
```
...
``` text showLineNumbers=false title=neo4j$
MATCH (c1:City{id:1})-[:HIGHWAY]->(dest) RETURN dest.name, dest.country
```
...
``` text showLineNumbers=false title=neo4j$
MATCH (start:City {id: 1})-[:HIGHWAY*1..]->(ville)
...
Le langage Cypher est très riche, n'hésitez pas à le découvrir par vous même en vous aidant de la documentation.

<Card title="Cypher" icon="pencil">
Utilisez la documentation officielle Neo4j pour déterminer les commandes à utiliser pour:

- supprimer la ville `Dublin` de votre graphe,

- afficher (sans doublon !) la liste des différents pays présents dans votre base de données (voyez l'utilisation de la sous-clause `WHERE` de la clause `MATCH`),

- afficher le nombre total de noeuds présents (voyez l'utilisation de la fonction `COUNT`).
