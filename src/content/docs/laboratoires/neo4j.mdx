...
Comme vous avez pu le constater ci-dessus, les requêtes SQL nécessaire pour parcourir le graphe deviennent vite longue et fastidieuse. L'utilisation d'un SGBD dédié à la manipulation des graphes prend alors ici tout son intérêt.
...
La commande Docker à utiliser est (remplacez évidemment le nom de user `g12345` par le vôtre et le mot de passe `myPassword` par un mot de passe de votre choix):

``` bash showLineNumbers=false
docker run --name MyNeo4j -p7474:7474 -p7687:7687 -d -e NEO4J_AUTH=g12345/myPassword neo4j:5
```
...
Connectez vous (au moyen du user/mot de passe fournis ci-dessus) et vérifiez que tout est bien en place.
