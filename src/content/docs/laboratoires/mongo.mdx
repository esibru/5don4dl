...
Remarquez que MongoDB assigne automatiquement un identifiant à chaque nouveau document.
...
- proposer une plue grande variété de types de données, par exemple le type `Date` (non supporté par le JSON standard, les dates sont encodées comme des `String` en JSON),
...
Pour utiliser le type de donnée date, par exemple, il suffit d'écrire:
