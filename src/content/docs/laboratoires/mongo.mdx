...
``` bash showLineNumbers=false
db.clients.insertOne({id: 34, name: "Louis"})
```

insère un nouveau clien d'identifiant 34 et de nom Louis dans la collections `clients` (celle-ci est créée si elle n'existe pas déjà).
